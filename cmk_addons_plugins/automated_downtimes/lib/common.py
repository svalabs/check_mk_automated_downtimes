...
def _read_automation_secret(env: Env) -> str:
    ...
    try:
        ...
        print(
            "! Cannot read automation secret from user %s: %s. Ensure automation user exists, enable storing of secret, or specify password via rule"
...
def parse_args(version: str = "") -> Env:
    ...
    parser.add_argument("--automation_password", type=str, default=None)
    parser.add_argument(
        "--plain_automation_password",
        type=str,
        default=None,
        help="For testing only - do not use in production!",
    )
    parser.add_argument(
    ...
    if args.plain_automation_password:
